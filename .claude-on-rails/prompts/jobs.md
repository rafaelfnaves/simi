...
## Project specifics — Solid Queue (no Sidekiq, no Redis)

- The queue adapter is **`solid_queue`**, Postgres-backed. In production the
  supervisor runs **inside Puma** (`SOLID_QUEUE_IN_PUMA: true` in
  `config/deploy.yml`); locally/standalone it runs via **`bin/jobs`**.
- Configure queues and workers in **`config/queue.yml`**; recurring/scheduled jobs
  go in **`config/recurring.yml`** (not `sidekiq.yml`, not sidekiq-cron, not a
  self-requeueing "cron" job).
- All jobs subclass `ApplicationJob`. Use `queue_as`; keep queue names in sync with
  `config/queue.yml`.
- `retry_on ... wait: :exponentially_longer` is deprecated — use
  `wait: :polynomially_longer` (or an explicit duration).
- Job specs use RSpec + `ActiveJob::TestHelper` (`have_enqueued_job`,
  `perform_enqueued_jobs`); coordinate with the **tests** agent.
...
class SendEmailJob < ApplicationJob
  retry_on Net::SMTPServerError, wait: :polynomially_longer, attempts: 5
...
1. **Queue Priority** — `config/queue.yml`
```yaml
production:
  dispatchers:
    - polling_interval: 1
  ...
  workers:
    - queues: [urgent, default, low]
      ...
      processes: 1
```
Solid Queue works the listed queues in order; put latency-sensitive work on
`urgent`.
