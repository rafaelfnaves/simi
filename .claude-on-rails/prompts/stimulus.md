...
- **JavaScript is served via importmap** (`importmap-rails`) — there is **no Node,
  no npm/yarn, no build step**. Add a dependency with `bin/importmap pin <pkg>`
  (writes to `config/importmap.rb`); vendor local helper modules under
  `app/javascript/` and `pin` them (or rely on `pin_all_from`).
- Stimulus controllers in `app/javascript/controllers/` auto-register via
  `stimulus-loading` + `pin_all_from` (see `controllers/index.js`).
- **Views are Phlex classes, not ERB** (`app/views/**/*.rb`). The ERB snippets
  below are illustrative only — in this project the markup and its `data-*`
  attributes are emitted from Phlex; the **views** agent owns that. You own the
  controller JS and Turbo Stream / broadcast behavior.
- Action Cable is backed by **`solid_cable`** (Postgres), not Redis.
